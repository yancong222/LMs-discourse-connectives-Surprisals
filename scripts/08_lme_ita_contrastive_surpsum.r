...
library(lmerTest)
# According to 
# the lmertest paper[https://www.jstatsoft.org/article/view/v082i13/0]
# is it simply an expansion to lme4.

## inter surp
gpt2Italian_avg = lmer(gpt2Italian_avg~Plausibility + Connective + Plausibility*Connective + gpt2Italian_tokens_len + (1|ITEM_ID), data = df)
summary(gpt2Italian_avg) 
gpt2Italian_sum = lmer(gpt2Italian_sum~Plausibility + Connective + Plausibility*Connective + gpt2Italian_tokens_len + (1|ITEM_ID), data = df)
summary(gpt2Italian_sum)

model2Italian_avg = lmer(model2Italian_avg~Plausibility + Connective + Plausibility*Connective + gpt2Italian_tokens_len + (1|ITEM_ID), data = df)
summary(model2Italian_avg) 
model2Italian_sum = lmer(model2Italian_sum~Plausibility + Connective + Plausibility*Connective + gpt2Italian_tokens_len + (1|ITEM_ID), data = df)
